 ...
 */
class StoredQueryMap final : private FileContentChecker
{
 public:
 ...
 private:
  ...
  std::shared_ptr<const StoredQueryHandlerBase> get_handler_by_file_name(
      const std::string& config_file_name) const;

  std::shared_ptr<const StoredQueryConfig> get_query_config_by_file_name(
